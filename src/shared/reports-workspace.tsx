...
function ReportNarrativeReader({ report, narrative }: { report: ReportDto; narrative: Record<string, unknown> | null }) {
  ...
  return <div className="report-structured-narrative" data-report-type={report.report_type}>
    ...
    {sources.length > 0 && <section className="report-narrative-block"><div className="report-subheading"><span className="eyebrow">Traceability</span><h3>Sources used</h3></div><ul className="report-source-reference-list">{sources.map((item, index) => <li key={index}><span>Evidence {index + 1}</span><strong>{reportSourceLabel(item, pack)}</strong></li>)}</ul></section>}
  </div>;
}
...
function reportSourceLabel(item: Record<string, unknown>, pack: Record<string, unknown>[]) {
  const claimId = item.claimId || item.claim_id;
  const claim = pack.flatMap((packItem) => asList(packItem.claims)).find((candidate) => candidate.claimId === claimId || candidate.claim_id === claimId);
  const sourceIds = asTextList(claim?.sourceArticleIds ?? claim?.source_article_ids);
  const citation = pack.flatMap((packItem) => asList(packItem.citations)).find((candidate) => sourceIds.includes(String(candidate.sourceArticleId || candidate.source_article_id)));
  const title = citation?.title || citation?.article_title;
  if (typeof title === "string" && title.trim()) return title.trim();
  const media = citation?.sourceName || citation?.source_name || citation?.media;
  if (typeof media === "string" && media.trim()) return media.trim();
  const id = citation?.sourceArticleId || citation?.source_article_id || item.sourceArticleId || item.source_article_id;
  if (typeof id === "string") { const provider = /^crawl:([^:]+):/i.exec(id)?.[1]; if (provider) return provider.replace(/[-_]+/g, " ").replace(/\b\w/g, (letter) => letter.toUpperCase()); }
