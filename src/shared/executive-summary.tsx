...
function ExecutiveSummaryBody() {
  ...
  return <div className="summary-page"><div className="page-context"><span className="supporting-text">The five issues that deserve executive attention in the selected period.</span><div className="period-selector" role="group" aria-label="Summary period">{([ ["24jam", "24 hours"], ["7hari", "7 days"], ["30hari", "30 days"] ] as const).map(([value, label]) => <button key={value} className={period === value ? "is-active" : ""} onClick={() => setPeriod(value)}>{label}</button>)}</div></div>{query.isLoading ? <SummaryLoading /> : query.isError ? <SummaryError error={query.error} onRetry={() => query.refetch()} /> : !query.data ? <SummaryEmpty /> : <SummaryContent data={query.data} period={period} />}</div>;
}
