...
type Source = { sourceArticleId: string; canonicalUrl: string | null; locale: string | null; title: string | null; sourceName: string | null; publishedAt: string | null };
...
type Citation = Source & { updatedAt: string | null };
...
function Timeline({ developments }: { developments: Development[] }) { return developments.length ? <div className="drawer-timeline">{developments.map((item) => <div className="timeline-item" key={item.developmentId}><span className="timeline-dot" /><div><strong>{item.developmentType}</strong><time>{formatDate(item.observedAt)}</time></div></div>)}</div> : <InlineEmpty text="No development timeline is available." />; }
...
function sourceMeta(source: Source) { return [source.sourceName || sourceHost(source.canonicalUrl), source.publishedAt ? formatDate(source.publishedAt) : null].filter(Boolean).join(" · "); }
function sourceHost(value: string | null) { if (!value) return null; try { return new URL(value).hostname.replace(/^www\./, ""); } catch { return null; } }
function SourceRow({ source, onPreview }: { source: Source; onPreview: (source: Source) => void }) { const available = Boolean(source.canonicalUrl); const title = source.title || "Article title unavailable"; const meta = sourceMeta(source) || "Source metadata unavailable"; return <div className="source-row">{available ? <a href={source.canonicalUrl as string} target="_blank" rel="noreferrer" className="source-link"><span className="source-copy"><strong className="source-title">{title}</strong><small className="source-meta">{meta}</small></span><em aria-label="Open article"><ExternalLink size={13} strokeWidth={2} aria-hidden="true" /></em></a> : <div className="source-unavailable"><span className="source-copy"><strong className="source-title">{title}</strong><small className="source-meta">{meta}</small></span><em>Link unavailable</em></div>}<button className="source-preview-button" onClick={() => onPreview(source)}>Preview</button></div>; }
function SourcePreview({ source, previewRef, onClose }: { source: Source; previewRef: React.RefObject<HTMLElement | null>; onClose: () => void }) { const available = Boolean(source.canonicalUrl); return <div className="source-preview-layer"><button className="source-preview-backdrop" onClick={onClose} aria-label="Close source preview" /><section ref={previewRef} className="source-preview-card" role="dialog" aria-modal="true" aria-label="Source preview"><button className="drawer-close" onClick={onClose} aria-label="Close source preview"><X size={18} strokeWidth={2} aria-hidden="true" /></button><div className="drawer-eyebrow">Source preview</div><h3>{source.title || "Article source"}</h3><span className="source-preview-meta">{sourceMeta(source) || "Source metadata unavailable"}</span>{available ? <><p>This preview is limited to canonical source metadata supplied by the backend.</p><a className="context-action" href={source.canonicalUrl as string} target="_blank" rel="noreferrer">Open canonical article <ExternalLink size={14} strokeWidth={2} aria-hidden="true" /></a></> : <><span className="source-unavailable-badge">Source unavailable</span><p>The canonical article link is not available for this source.</p></>}</section></div>; }
function CitationRefs({ ids, citations }: { ids: string[]; citations: Citation[] }) { const byId = new Map(citations.map((citation) => [citation.sourceArticleId, citation])); const visible = ids.map((id) => byId.get(id)).filter((citation): citation is Citation => Boolean(citation)); return visible.length ? <div className="citation-refs">{visible.map((citation) => <span key={citation.sourceArticleId}>{citation.sourceName || sourceHost(citation.canonicalUrl) || "Source"}</span>)}</div> : null; }
