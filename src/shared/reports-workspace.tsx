...
function ReportNarrativeReader({ report, narrative }: { report: ReportDto; narrative: Record<string, unknown> | null }) {
  ...
  return <div className="report-structured-narrative" data-report-type={report.report_type}>
    ...
    {sources.length > 0 && <section className="report-narrative-block"><div className="report-subheading"><span className="eyebrow">Traceability</span><h3>Sources used</h3></div><ul className="report-source-reference-list">{sources.map((item, index) => <li key={index}><span>{String(item.claimId || item.claim_id || `Source ${index + 1}`)}</span><code>{String(item.sourceArticleId || item.source_article_id || item.text || "")}</code></li>)}</ul></section>}
  </div>;
}
