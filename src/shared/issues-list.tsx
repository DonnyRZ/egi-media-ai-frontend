...
function IssuesListBody() {
  ...
  return <div className="issues-page">
    <div className="page-context"><span className="supporting-text">The complete company issue set, beyond the Executive Summary Top 5.</span><span className="issues-scope-badge">Company scoped</span></div>
    <div className="issues-toolbar">
      <label className="issues-search"><Search size={16} strokeWidth={2} aria-hidden="true" /><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Search title or one-liner..." aria-label="Search issues" /></label>
    ...
    {query.isLoading ? <IssuesLoading /> : query.isError ? <IssuesError error={query.error} onRetry={() => query.refetch()} /> : !query.data || query.data.items.length === 0 ? <IssuesEmpty filtered={hasFilters} onReset={resetFilters} /> : <><div className="issues-list">{query.data.items.map((issue) => <article className="issue-list-card" key={issue.id} role="button" tabIndex={0} onClick={() => openIssue(issue.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openIssue(issue.id); } }}><div className="issue-list-copy"><h2>{issue.title}</h2><p>{issue.oneLiner || "No one-liner is available for this issue yet."}</p><div className="badge-row issue-list-meta"><PriorityBadge value={issue.priority} /><StatusBadge value={issue.status} /></div></div><div className="issue-list-side"><span className="timestamp">Updated {formatDate(issue.lastDevelopedAt)}</span></div></article>)}</div><Pagination page={page} totalPages={totalPages} onPage={setPage} total={query.data.pagination.total} /></>}
  </div>;
}

function PriorityBadge({ value }: { value: "tinggi" | "sedang" | "rendah" | null }) { return <span className={`meta-tag meta-priority meta-priority-${value ?? "rendah"}`}>{value ?? "unprioritized"}</span>; }
function StatusBadge({ value }: { value: "baru" | "berkembang" | "dipantau" | "selesai" }) { return <span className={`meta-tag meta-status meta-status-${value}`}>{value}</span>; }
function Pagination({ page, totalPages, total, onPage }: { page: number; totalPages: number; total: number; onPage: (page: number) => void }) { return <div className="issues-pagination"><span>{total} issues · Page {page} of {totalPages}</span><div><button disabled={page <= 1} onClick={() => onPage(page - 1)} aria-label="Previous page"><ChevronLeft size={16} strokeWidth={2} aria-hidden="true" /></button><button disabled={page >= totalPages} onClick={() => onPage(page + 1)} aria-label="Next page"><ChevronRight size={16} strokeWidth={2} aria-hidden="true" /></button></div></div>; }
...
function IssuesEmpty({ filtered, onReset }: { filtered: boolean; onReset: () => void }) { return <div className="issues-empty"><div className="summary-empty-mark"><Inbox size={20} strokeWidth={2} aria-hidden="true" /></div><h2>{filtered ? "No issues match these filters" : "No issues yet"}</h2><p>{filtered ? "Try a broader search or remove one of the filters." : "There are no issue records available for this company scope."}</p>{filtered && <button className="context-action" onClick={onReset}>Clear filters</button>}</div>; }
function IssuesError({ error, onRetry }: { error: unknown; onRetry: () => void }) { const message = isAxiosError<{ error?: { message?: string } }>(error) ? error.response?.data?.error?.message ?? "The issue list could not be reached." : "The issue list could not be loaded."; return <div className="issues-empty"><div className="summary-empty-mark"><AlertCircle size={20} strokeWidth={2} aria-hidden="true" /></div><h2>Issues unavailable</h2><p>{message}</p><button className="context-action" onClick={onRetry}>Try again</button></div>; }
