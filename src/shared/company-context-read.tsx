...
function ListValue({ value }: { value: unknown[] }) { return value.length ? <ul className="context-value-list">{value.map((item, index) => <li key={`${String(item)}-${index}`}>{String(item)}</li>)}</ul> : <span className="context-empty-value">Not provided</span>; }
